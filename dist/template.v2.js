(function (factory) {
  ...
  const katexJSURL = 'https://cdn.jsdelivr.net/npm/katex@0.16.7/dist/katex.min.js';
  const katexCSSTag = '<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.7/dist/katex.min.css">';
  const myStyles = '<link rel="stylesheet" href="styles.css">';
...
<style>
  :host {
    font-size: 1.25rem;
    line-height: 1.6em;
    color: rgba(255, 255, 255, 0.85); /* Light text for dark mode */
    -webkit-font-smoothing: antialiased;
  ...
  ::slotted(p) {
    ...
    color: rgba(255, 255, 255, 0.85); /* Ensures paragraphs are light-colored */
  }
  ...
  <div class="byline grid">
    <div class="authors-affiliations grid">
      ...
      ${frontMatter.authors.map(author => `
        ...
        <p class="affiliation">
          <span class="affiliation">${author.affiliations[0].name}</span>
        </p>
      `).join('')}
    </div>
    <div>
      <h3>Published</h3>
      <p><a href="https://openreview.net/forum?id=Xo1Yqyw7Yx">📢 NeurIPS 2024</a></p>
    </div>
    <div>
      <h3>Links</h3>
      <p>
...
.citation {
  color: hsla(206, 90%, 80%, 0.7); /* Lighter blue for dark mode */
}
...
.citation-number {
  ...
  font-size: 75%;
  color: hsla(206, 90%, 80%, 0.7); /* Matches citation text */
  display: inline-block;
  line-height: 1.1em;
  text-align: center;
  position: relative;
...
figcaption .citation-number {
  ...
  top: -2px;
  line-height: 1em;
  color: hsla(206, 90%, 80%, 0.7); /* Consistent with citations */
}
...
ul li {
  ...
  border-bottom: 1px solid rgba(255, 255, 255, 0.1); /* Subtle divider for dark mode */
}
...
code {
  white-space: nowrap;
  background: rgba(255, 255, 255, 0.05); /* Subtle light background for dark mode */
  border-radius: 2px;
  padding: 4px 7px;
  font-size: 15px;
  color: rgba(255, 255, 255, 0.85); /* Light text for dark mode */
...
pre code {
  display: block;
  border-left: 2px solid rgba(255, 255, 255, 0.2); /* Light border for dark mode */
  padding: 0 0 0 36px;
  background: rgba(255, 255, 255, 0.05); /* Match inline code background */
  color: rgba(255, 255, 255, 0.85); /* Ensure consistency with inline code */
}
...
sup {
  ...
  top: -.5em;
  vertical-align: baseline;
  color: rgba(255, 255, 255, 0.7); /* Subtle light tone for superscript */
}

span {
  color: hsla(206, 90%, 70%, 0.85); /* Lighter blue for better contrast in dark mode */
  cursor: default;
}

.footnote-container {
  padding: 10px;
  background-color: rgba(255, 255, 255, 0.05); /* Light background for separation in dark mode */
...
.panel {
  ...
  left: 0;
  width: 100%;
  border: 1px solid rgba(255, 255, 255, 0.2); /* Light border for dark mode */
  background-color: rgba(20, 20, 20, 0.95); /* Dark background with slight transparency */
  box-shadow: 0 0 7px rgba(0, 0, 0, 0.5); /* Subtle shadow for depth */
  border-radius: 4px;
  box-sizing: border-box;
...
`, false);
  ...
  class TOC2 extends HTMLElement {

    static get is() { return 'd-toc-app'; }

    connectedCallback() {
      if (!this.getAttribute('prerendered')) {
        window.onload = () => {
          const article = document.querySelector('d-appendix');
          const headings = article.querySelectorAll('h2, h3');
          renderTOC(this, headings);
        };
      }
...
`;
  ...
  const initialize = function() {
    ...
    const components = [
      ...
      Footnote, FootnoteList, FrontMatter$1, HoverBox, Title, DMath, References, TOC, TOC2, Figure,
      Slider, Interstitial
    ];
